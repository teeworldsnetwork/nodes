	...
	if (ResponseCode == 200)
	{
		...
		for (unsigned int i = 0; i < pServerList->u.array.length; i++)
		{
			...
			for (unsigned int j = 0; j < pEntry->u.object.length; j++)
			{
				...
				{
					char aPort[8];
					str_format(aPort, sizeof(aPort), ":%d", str_toint(pItem->u.string.ptr));
				...
				{
					int Flags = str_toint(pItem->u.string.ptr);
					Info.m_Flags = 0;
					...
					Info.m_ServerLevel = clamp<int>(str_toint(pItem->u.string.ptr), SERVERINFO_LEVEL_MIN, SERVERINFO_LEVEL_MAX);
				else if (!str_comp_nocase(pEntry->u.object.values[j].name, "players_online"))
					Info.m_NumPlayers = str_toint(pItem->u.string.ptr);
				else if (!str_comp_nocase(pEntry->u.object.values[j].name, "players_max"))
					Info.m_MaxPlayers = str_toint(pItem->u.string.ptr);
				else if (!str_comp_nocase(pEntry->u.object.values[j].name, "clients_online"))
					Info.m_NumClients = str_toint(pItem->u.string.ptr);
				else if (!str_comp_nocase(pEntry->u.object.values[j].name, "clients_max"))
					Info.m_MaxClients = str_toint(pItem->u.string.ptr);
					...
					for (unsigned int c = 0; c < pItem->u.array.length; c++)
					{
						...
						for (unsigned int p = 0; p < pClientProp->u.object.length; p++)
						{
							if (!str_comp_nocase(pClientProp->u.object.values[p].name, "name"))
								...
								Info.m_aClients[i].m_Country = str_toint(pClientProp->u.object.values[p].value->u.string.ptr);
							else if (!str_comp_nocase(pClientProp->u.object.values[p].name, "score"))
								Info.m_aClients[i].m_Score = str_toint(pClientProp->u.object.values[p].value->u.string.ptr);
							else if (!str_comp_nocase(pClientProp->u.object.values[p].name, "type"))
								Info.m_aClients[i].m_PlayerType = str_toint(pClientProp->u.object.values[p].value->u.string.ptr);
						}
					}
				}
			}
