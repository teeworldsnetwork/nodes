...
public:
	...
	void InitInterfaces(CConfig *pConfig, IConsole *pConsole, IGameServer *pGameServer, IEngineMap *pMap, IStorage *pStorage, IHttp *pHttp);
	int Run();
...
private:
	...
	int64 m_LastServerRegistration;

	void HandleServerRegistration();
	void DoServerRegistration();

	static void ServerRegisterCallback(void* pUser, int ResponseCode, std::string Response);
};
