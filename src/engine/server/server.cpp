...
void CServer::PumpNetwork()
{
	...
	while(m_NetServer.Recv(&Packet, &ResponseToken))
	{
		...
		{
			// todo: udp serverinfo only for local clients. need to add local ip range check
...
void CServer::InitInterfaces(CConfig *pConfig, IConsole *pConsole, IGameServer *pGameServer, IEngineMap *pMap, IStorage *pStorage, IHttp *pHttp)
{
	m_pConfig = pConfig;
	m_pConsole = pConsole;
...
int main(int argc, const char **argv) // ignore_convention
{
...

	{
		...
		RegisterFail = RegisterFail || !pKernel->RegisterInterface(pHttp);
	...
	pServer->InitInterfaces(pConfigManager->Values(), pConsole, pGameServer, pEngineMap, pStorage, pHttp);
	if(!UseDefaultConfig)
	{
...
void CServer::HandleServerRegistration()
{
	if (!Config()->m_SvRegister || (m_LastServerRegistration && m_LastServerRegistration + time_freq() * 50 > time_get()))
		return;
