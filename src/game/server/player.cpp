...
void CPlayer::Snap(int SnappingClient)
{
	...
	if (GetCharacter() && GetCharacter()->m_Ill)
		pPlayerInfo->m_PlayerFlags |= PLAYERFLAG_ILL;
...
{
	((CGameControllerNODES*)GameServer()->m_pController)->LeaveSpawnQueue(this);
	KillCharacter();
...
void CPlayer::OnDirectInput(CNetObj_PlayerInput *NewInput)
{
	...
	{
		((CGameControllerNODES*)GameServer()->m_pController)->EnterSpawnQueue(this);
		m_Spawning = true;
	}
	...
	if (!m_pCharacter && m_Team >= 0 && (NewInput->m_NextWeapon > m_LastInput.m_NextWeapon) && pNodes->m_aBuildingsCount[m_Team] > 0)
	{
		m_SelectSpawn = true;

		bool Overflow = false;
		int w = m_WantedSpawn + 1;

		while (w != m_WantedSpawn)
		{
			if (w >= pNodes->m_aBuildingsCount[m_Team])
			{
				if (Overflow)
					break;
			...
			if (pNodes->m_apBuildings[m_Team][w]->Type() == B_SPAWN && pNodes->m_apBuildings[m_Team][w]->Alive() && pNodes->m_apBuildings[m_Team][w]->Power())
				break;
	...
	else if (!m_pCharacter && m_Team >= 0 && (NewInput->m_PrevWeapon > m_LastInput.m_PrevWeapon) && pNodes->m_aBuildingsCount[m_Team] > 0)
	{
		m_SelectSpawn = true;
		...
		while (w != m_WantedSpawn)
		{
			if (w < 0)
			{
				...
				w = pNodes->m_aBuildingsCount[m_Team] - 1;
				Overflow = true;
			}
	...
	if (Updated && pNodes->m_apBuildings[m_Team][m_WantedSpawn] && pNodes->m_apBuildings[m_Team][m_WantedSpawn]->Type() == B_SPAWN && pNodes->m_apBuildings[m_Team][m_WantedSpawn]->Alive() && pNodes->m_apBuildings[m_Team][m_WantedSpawn]->Power())
	{
		vec2 Pos = pNodes->m_apBuildings[m_Team][m_WantedSpawn]->GetPos();
		GameServer()->PositionUpdate(Pos);

		m_ViewPos = Pos;
		m_Spawning = true;
		pNodes->LeaveSpawnQueue(this);
	}

	if (!m_pCharacter && m_Team == -1)
		m_ViewPos = vec2(NewInput->m_TargetX, NewInput->m_TargetY);

	mem_copy(&m_LastInput, NewInput, sizeof(m_LastInput));
}
...
{
	vec2 SpawnPos = vec2(100, -60);

	CGameControllerNODES* pNodes = ((CGameControllerNODES*)GameServer()->m_pController);
	if (m_Team == TEAM_SPECTATORS)
		return;

	if (!pNodes->CanSpawnNodes(this, m_Team, &SpawnPos))
		return;

	int Spawn = -1;
	if (pNodes->m_apBuildings[m_Team][m_WantedSpawn] && pNodes->m_apBuildings[m_Team][m_WantedSpawn]->Type() == B_SPAWN && pNodes->m_apBuildings[m_Team][m_WantedSpawn]->Alive() && pNodes->m_apBuildings[m_Team][m_WantedSpawn]->Power())
		Spawn = m_WantedSpawn;
	else
	{
		for (int i = 0; i < pNodes->m_aBuildingsCount[m_Team]; i++)
		{
			if (pNodes->m_apBuildings[m_Team][i] && pNodes->m_apBuildings[m_Team][i]->Type() == B_SPAWN && pNodes->m_apBuildings[m_Team][i]->Alive() && pNodes->m_apBuildings[m_Team][i]->Power())
			{
				Spawn = i;
				break;
			}
	...
	SpawnPos = vec2(pNodes->m_apBuildings[m_Team][Spawn]->GetPos().x, pNodes->m_apBuildings[m_Team][Spawn]->GetPos().y - 80);
