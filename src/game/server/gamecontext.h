...
private:
	...
	static void ConSetTechLevel(IConsole::IResult* pResult, void* pUserData);
	static void ConSetBuildPoints(IConsole::IResult* pResult, void* pUserData);
	static void ConSetRage(IConsole::IResult* pResult, void* pUserData);
	static void ConSetHealth(IConsole::IResult* pResult, void* pUserData);
	static void ConSetArmor(IConsole::IResult* pResult, void* pUserData);

public:

	void PositionUpdate(vec2 Pos);
	void CreateWrenchHit(vec2 Pos);
	void DoTeamSound(int Sound, int Team);
	void CreateEffect(vec2 Pos, int EffectID);
	void CreateSoundGlobal(int Sound, int Target);
	void SendBuildEmoticon(int ClientID, int Building);
	void SendChatMessage(int To, const char* pText, ...);
	void CreateDamageInd(vec2 Pos, float Angle, int Amount);

	int GetTeam(int ClientID) const;
};
