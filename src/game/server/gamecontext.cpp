...
void CGameContext::CreateExplosion(vec2 Pos, int Owner, int Weapon, int MaxDamage)
{
	...
	for(int i = 0; i < Num; i++)
	{
		...
		if((int)(Factor * MaxDamage))
			apEnts[i]->TakeDamage(Force * Factor, Diff*-1, (int)(Factor * MaxDamage), Owner, Weapon, -1);
	}

	// deal building damage
	CBuilding* apBuildings[64];
	Num = m_World.FindEntities(Pos, Radius, (CEntity**)apBuildings, 64, CGameWorld::ENTTYPE_BUILDING);
	for (int i = 0; i < Num; i++)
	{
		...
		if (l)
			Force = normalize(Diff) * MaxForce;
		float Factor = 1 - clamp((l - InnerRadius) / (Radius - InnerRadius), 0.0f, 1.0f);
		apBuildings[i]->TakeDamage(Force * Factor, (int)(Factor * MaxDamage), Owner, -1, Weapon);
	}
}
...
void CGameContext::PositionUpdate(vec2 Pos)
{
	// create the event
	CNetEvent_PositionUpdate* pEvent = (CNetEvent_PositionUpdate*)m_Events.Create(NETEVENTTYPE_POSITIONUPDATE, sizeof(CNetEvent_PositionUpdate));
	if (pEvent)
	{
		pEvent->m_X = (int)Pos.x;
		pEvent->m_Y = (int)Pos.y;
...
void CGameContext::SendBuildEmoticon(int ClientID, int Building)
{
	CNetMsg_Sv_BuildEmoticon Msg;
	Msg.m_ClientID = ClientID;
...
void CGameContext::SendChatMessage(int To, const char* pText, ...)
{
	char buffer[256];
	...
	va_list ap;
	va_start(ap, pText);
	_vsprintf_p(buffer, buffer_size, pText, ap);
	va_end(ap);
#else
	va_list ap;
	va_start(ap, pText);
	vsnprintf(buffer, buffer_size, pText, ap);
	va_end(ap);
#endif
	...
	Msg.m_Mode = CHAT_ALL;
	Msg.m_ClientID = -1;
	Msg.m_pMessage = buffer;
	Server()->SendPackMsg(&Msg, MSGFLAG_VITAL, To);
}

void CGameContext::DoTeamSound(int Sound, int Team)
{
	for (int i = 0; i < MAX_CLIENTS; i++)
	{
...
{
	// create the event
	CNetEvent_WrenchHit* pEvent = (CNetEvent_WrenchHit*)m_Events.Create(NETEVENTTYPE_WRENCHHIT, sizeof(CNetEvent_WrenchHit));
	if (pEvent)
	{
		pEvent->m_X = (int)Pos.x;
		pEvent->m_Y = (int)Pos.y;
...
void CGameContext::CreateEffect(vec2 Pos, int EffectID)
{
	// create the event
...
void CGameContext::OnClientConnected(int ClientID, bool Dummy, bool AsSpec)
{
	...
	Msg.m_TechRed = ((CGameControllerNODES*)m_pController)->m_aTechLevel[TEAM_RED];
	Msg.m_TechBlue = ((CGameControllerNODES*)m_pController)->m_aTechLevel[TEAM_BLUE];
	Server()->SendPackMsg(&Msg, MSGFLAG_VITAL, ClientID);
...
void CGameContext::OnMessage(int MsgID, CUnpacker *pUnpacker, int ClientID)
{
	...
	if(Server()->ClientIngame(ClientID))
	{
		...
		else if (MsgID == NETMSGTYPE_CL_BUILDBUILDING && !m_World.m_Paused)
		{
			CNetMsg_Cl_BuildBuilding* pMsg = (CNetMsg_Cl_BuildBuilding*)pRawMsg;

			// todo: own spamprotection for building?
			if (Config()->m_SvSpamprotection && pPlayer->m_LastEmote && pPlayer->m_LastEmote + Server()->TickSpeed() * 3 > Server()->Tick())
				return;

			pPlayer->m_LastEmote = Server()->Tick();

			int Building = pMsg->m_Building;
			CCharacter* pOwner = GetPlayerChar(pPlayer->GetCID());
			if (pOwner)
				pOwner->BuildingSelect(Building);
...
void CGameContext::ConSetTechLevel(IConsole::IResult* pResult, void* pUserData)
{
	CGameContext* pSelf = (CGameContext*)pUserData;
	...
	if (Team != TEAM_RED && Team != TEAM_BLUE)
		str_format(aBuf, sizeof(aBuf), "Invalid Team. Choose between 0(Team red) -  1(Team blue)");
	else if (Value <= 0 || Value > 3)
		str_format(aBuf, sizeof(aBuf), "Invalid Techlevel. Choose between 1 - 3");
	else
	{
		((CGameControllerNODES*)pSelf->m_pController)->SetTechLevel(Team, Value);
		str_format(aBuf, sizeof(aBuf), "Successful. Team %s -> %d", Team == TEAM_RED ? "Red" : "Blue", Value);
	}

	pSelf->Console()->Print(IConsole::OUTPUT_LEVEL_STANDARD, "nodes", aBuf);
}

void CGameContext::ConSetBuildPoints(IConsole::IResult* pResult, void* pUserData)
{
	CGameContext* pSelf = (CGameContext*)pUserData;
	...
	if (Team != TEAM_RED && Team != TEAM_BLUE)
		...
		str_format(aBuf, sizeof(aBuf), "Invalid Buildpoints. Choose between 0 - 100000");
	else
	{
		((CGameControllerNODES*)pSelf->m_pController)->SetBuildPoints(Team, Value);
		str_format(aBuf, sizeof(aBuf), "Successful. Team %s -> %d", Team == TEAM_RED ? "Red" : "Blue", Value);
	}
...
void CGameContext::ConSetRage(IConsole::IResult* pResult, void* pUserData)
{
	CGameContext* pSelf = (CGameContext*)pUserData;

	int ClientID = clamp(pResult->GetInteger(0), 0, (int)MAX_CLIENTS - 1);
	int Value = clamp(pResult->GetInteger(1), 0, 100);

	CCharacter* pChr = pSelf->GetPlayerChar(ClientID);
	if (!pChr)
		return;

	pChr->GetPlayer()->m_Rage = Value;
}

void CGameContext::ConSetHealth(IConsole::IResult* pResult, void* pUserData)
{
	CGameContext* pSelf = (CGameContext*)pUserData;
...
void CGameContext::OnConsoleInit()
{
	...
	// test cmds
	Console()->Register("set_techlevel", "ii", CFGFLAG_SERVER, ConSetTechLevel, this, "Set Techlevel");
	Console()->Register("set_buildpoints", "ii", CFGFLAG_SERVER, ConSetBuildPoints, this, "Set Buildpoints");
	Console()->Register("set_rage", "ii", CFGFLAG_SERVER, ConSetRage, this, "Set Ragelevel");
	Console()->Register("set_health", "ii", CFGFLAG_SERVER, ConSetHealth, this, "Set Health");
	Console()->Register("set_armor", "ii", CFGFLAG_SERVER, ConSetArmor, this, "Set Armor");
}
...
int CGameContext::GetTeam(int ClientID) const
{
