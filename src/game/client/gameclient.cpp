...
void CGameClient::OnMessage(int MsgId, CUnpacker *pUnpacker)
{
	...
	else if (MsgId == NETMSGTYPE_SV_TECHLEVELUPDATE)
	{
		...
		m_aTechLevel[TEAM_RED] = pMsg->m_TechRed;
		m_aTechLevel[TEAM_BLUE] = pMsg->m_TechBlue;

		Collision()->SetTechLevel(TEAM_RED, m_aTechLevel[0]);
		Collision()->SetTechLevel(TEAM_BLUE, m_aTechLevel[1]);
	}
	...
	else if (MsgId == NETMSGTYPE_SV_BUILDEMOTICON)
	{
		CNetMsg_Sv_BuildEmoticon* pMsg = (CNetMsg_Sv_BuildEmoticon*)pRawMsg;

		// apply
		m_aClients[pMsg->m_ClientID].m_Emoticon = 16 + pMsg->m_Building; // hack
		m_aClients[pMsg->m_ClientID].m_EmoticonStart = Client()->GameTick();
	}
}
...
void CGameClient::ProcessEvents()
{
	...
	for(int Index = 0; Index < Num; Index++)
	{
		...
		else if (Item.m_Type == NETEVENTTYPE_DAMAGEIND) // todo: replace with new damage event
		{
			CNetEvent_DamageInd* ev = (CNetEvent_DamageInd*)pData;
			m_pDamageind->Create(vec2(ev->m_X, ev->m_Y), direction(ev->m_Angle / 256.0f));
		}
		else if (Item.m_Type == NETEVENTTYPE_WRENCHHIT)
		{
			CNetEvent_PositionUpdate* ev = (CNetEvent_PositionUpdate*)pData;
			m_pEffects->WrenchHit(vec2(ev->m_X, ev->m_Y));
		}
		else if (Item.m_Type == NETEVENTTYPE_EFFECT)
		{
			...
			{
			case EFFECT_FALLOUT:
				m_pEffects->Fallout(vec2(ev->m_X, ev->m_Y));
				break;
			case EFFECT_RAMPAGE:
				m_pEffects->Emp(vec2(ev->m_X, ev->m_Y));
				break;
			case EFFECT_FREEZE:
				m_pEffects->IceExpl(vec2(ev->m_X, ev->m_Y));
				break;
			}
		}
...
void CGameClient::OnNewSnapshot()
{
	...
	// go trough all the items in the snapshot and gather the info we want
	{
		...
		for(int i = 0; i < Num; i++)
		{
			...
			{
				m_Snap.m_apBuildings[Item.m_ID % 2] = (const CNetObj_Building*)pData;
			}
		}
	}
