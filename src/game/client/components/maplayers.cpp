			...
			if (!Render)
			{
				if (IsGameLayer)
				{
					...
					for (int i = 0; i < pLayers->GameLayer()->m_Width * pLayers->GameLayer()->m_Height; i++)
					{
						...
						{
							if (Index == (CCollision::COLFLAG_BLUE | CCollision::COLFLAG_RED) && m_pClient->m_aTechLevel[0] < 2 && m_pClient->m_aTechLevel[1] < 2)
								Sprite = SPRITE_BLOCK_WHITE_2;
							else if (Index == (CCollision::COLFLAG_BLUE | CCollision::COLFLAG_RED | CCollision::COLFLAG_TL3) && m_pClient->m_aTechLevel[0] < 3 && m_pClient->m_aTechLevel[1] < 3)
								Sprite = SPRITE_BLOCK_WHITE_3;
							else if (Index == CCollision::COLFLAG_RED && m_pClient->m_aTechLevel[0] < 2)
								Sprite = SPRITE_BLOCK_RED_2;
							else if (Index == (CCollision::COLFLAG_RED | CCollision::COLFLAG_TL3) && m_pClient->m_aTechLevel[0] < 3)
								Sprite = SPRITE_BLOCK_RED_3;
							else if (Index == CCollision::COLFLAG_BLUE && m_pClient->m_aTechLevel[1] < 2)
								Sprite = SPRITE_BLOCK_BLUE_2;
							else if (Index == (CCollision::COLFLAG_BLUE | CCollision::COLFLAG_TL3) && m_pClient->m_aTechLevel[1] < 3)
