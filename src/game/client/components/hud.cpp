...
void CHud::RenderCursor()
{
	...
	// render cursor
	if (m_pClient->m_Snap.m_pLocalCharacter->m_CrateBuff == CRATE_BASEBALL && m_pClient->m_Snap.m_pLocalCharacter->m_Weapon == WEAPON_HAMMER)
...
void CHud::RenderHealthAndAmmo(const CNetObj_Character *pCharacter)
{
	...
	Graphics()->TextureSet(g_pData->m_aImages[IMAGE_GAME].m_Id);
	Graphics()->MapScreen(0.0f, 0.0f, m_Width, 300);
	Graphics()->QuadsBegin();

	// if weaponstage is active, put a "glow" around the stage ammo
	RenderTools()->SelectSprite(g_pData->m_Weapons.m_aId[pCharacter->m_Weapon % NUM_WEAPONS].m_pSpriteProj);
	for (int i = 0; i < min(pCharacter->m_AmmoCount, 10); i++)
	{
		IGraphics::CQuadItem QuadItem(x + i * 12, y + 20, 10, 10);
		Graphics()->QuadsDrawTL(&QuadItem, 1);
	}
	...
	IGraphics::CQuadItem QuadItem(5, 5, 36, 36);
	Graphics()->QuadsDrawTL(&QuadItem, 1);
	...
	if (Spr >= 0)
	{
		...
		IGraphics::CQuadItem QuadItem(11, 11, 24, 24);
		Graphics()->QuadsDrawTL(&QuadItem, 1);
	}
	...
	// render health
	RenderTools()->SelectSprite((pCharacter->m_Health < 1 ? SPRITE_GUI_EMPTY_START : SPRITE_GUI_HEALTH_START));
	IGraphics::CQuadItem QuadItem2(x, y, 12, 12);
	Graphics()->QuadsDrawTL(&QuadItem2, 1);

	RenderTools()->SelectSprite(SPRITE_GUI_HEALTH);
	for (; h < min(pCharacter->m_Health, 9); h++)
	{
		IGraphics::CQuadItem QuadItem(x + h * 12, y, 12, 12);
		Graphics()->QuadsDrawTL(&QuadItem, 1);
	}
	...
	RenderTools()->SelectSprite((pCharacter->m_Health < 10 ? SPRITE_GUI_EMPTY_END : SPRITE_GUI_HEALTH_END));
	IGraphics::CQuadItem QuadItem3(x + 9 * 12, y, 12, 12);
	Graphics()->QuadsDrawTL(&QuadItem3, 1);

	// render armor
	h = 1;

	RenderTools()->SelectSprite((pCharacter->m_Armor < 1 ? SPRITE_GUI_EMPTY_START : SPRITE_GUI_ARMOR_START));
	IGraphics::CQuadItem QuadItem4(x, y + 10, 12, 12);
	Graphics()->QuadsDrawTL(&QuadItem4, 1);

	RenderTools()->SelectSprite(SPRITE_GUI_ARMOR);
	for (; h < min(pCharacter->m_Armor, 9); h++)
	{
		IGraphics::CQuadItem QuadItem(x + h * 12, y + 10, 12, 12);
		Graphics()->QuadsDrawTL(&QuadItem, 1);
	}
	...
	RenderTools()->SelectSprite((pCharacter->m_Armor < 10 ? SPRITE_GUI_EMPTY_END : SPRITE_GUI_ARMOR_END));
	IGraphics::CQuadItem QuadItem5(x + 9 * 12, y + 10, 12, 12);
	Graphics()->QuadsDrawTL(&QuadItem5, 1);
	Graphics()->QuadsEnd();
}
