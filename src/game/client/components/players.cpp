	...
	)
{
	...
	// draw gun
	{
		...
		if (Player.m_Weapon == WEAPON_HAMMER)
		{
			if (Player.m_CrateBuff == CRATE_BASEBALL)
				RenderTools()->SelectSprite(SPRITE_WEAPON_BASEBALL, Direction.x < 0 ? SPRITE_FLAG_FLIP_Y : 0);
	...
	if (pInfo.m_PlayerFlags & PLAYERFLAG_ILL)
	{
		/*
		SKINPART_BODY=0,
		SKINPART_MARKING,
		...
		*/
		...
		m_pClient->m_aClients[ClientID].m_Emoticon &= 0x7FFFFFFF;
		RenderInfo.m_aColors[SKINPART_BODY] = vec4(0.0f, 0.75f, 0.0f, 1.0f);
		RenderInfo.m_aColors[SKINPART_DECORATION] = vec4(0.0f, 0.75f, 0.0f, 1.0f);
		RenderInfo.m_aColors[SKINPART_HANDS] = vec4(0.0f, 0.75f, 0.0f, 1.0f);
		RenderInfo.m_aColors[SKINPART_FEET] = vec4(0.0f, 0.75f, 0.0f, 1.0f);

		if (Client()->GameTick() % 100 == 0)
			m_pClient->m_pEffects->Fallout2(Position);
	}

	bool IsTeamplay = (m_pClient->m_GameInfo.m_GameFlags & GAMEFLAG_TEAMS) != 0;

	if (Player.m_Effect & EFFECT_RAMPAGE)
	{
		...
		if (Skin != -1)
		{
			...
			for (int p = 0; p < NUM_SKINPARTS; p++)
			{
				...
				if (IsTeamplay)
				{
					...
					int ColorVal = m_pClient->m_pSkins->GetTeamColor(true, pNinja->m_aPartColors[p], m_pClient->m_aClients[ClientID].m_Team, p);
		...
		m_pClient->m_pEffects->PowerupShine(Position + (Direction.x < 0 ? vec2(32, 0) : vec2(-32, 0)), vec2(32, 12));
	}

	if (Player.m_Effect & EFFECT_BOMB)
	{
		int Skin = m_pClient->m_pSkins->Find("bomb", true);
		if (Skin != -1)
		{
			const CSkins::CSkin* pBomb = m_pClient->m_pSkins->Get(Skin);
			RenderInfo.m_aTextures[SKINPART_BODY] = pBomb->m_apParts[SKINPART_BODY]->m_ColorTexture;
		}
	}
	...
	// render effects
	if (Player.m_Effect & EFFECT_HEALING)
	{
		Graphics()->TextureSet(g_pData->m_aImages[IMAGE_FX_HOT].m_Id);
		Graphics()->QuadsBegin();
		RenderTools()->SelectSprite(SPRITE_FX_HOT_0 + (Client()->GameTick() % 36) / 3);
		IGraphics::CQuadItem QuadItem(Position.x, Position.y, 96, 96);
		Graphics()->QuadsDraw(&QuadItem, 1);
		Graphics()->QuadsEnd();
	}
	...
	if (Player.m_Effect & EFFECT_BOMB)
	{
		Graphics()->TextureSet(g_pData->m_aImages[IMAGE_FX_BOMB].m_Id);
		Graphics()->QuadsBegin();
		RenderTools()->SelectSprite(SPRITE_FX_BOMB_0 + (Client()->GameTick() / 2 % 4));
		IGraphics::CQuadItem QuadItem(Position.x + 12, Position.y - 43, 64, 64);
		Graphics()->QuadsDraw(&QuadItem, 1);
		Graphics()->QuadsEnd();
	}

	// render helmet
	if (Player.m_CrateBuff == CRATE_CONSTRUCTION)
	{
		Graphics()->TextureSet(g_pData->m_aImages[IMAGE_HELMETS].m_Id);
		Graphics()->QuadsBegin();
		RenderTools()->SelectSprite(SPRITE_HELMET_CONSTRUCTION, Direction.x > 0 ? SPRITE_FLAG_FLIP_X : 0);

		int AnimState = (Client()->GameTick() - Player.m_CrateBuffTick);
		IGraphics::CQuadItem QuadItem(Position.x, Position.y - 10, (AnimState <= 10 ? AnimState / 10.0f : 1) * 64, 64);
		Graphics()->QuadsDraw(&QuadItem, 1);
		Graphics()->QuadsEnd();
	}
	else if (Player.m_CrateBuff == CRATE_BASEBALL)
	{
		Graphics()->TextureSet(g_pData->m_aImages[IMAGE_HELMETS].m_Id);
		Graphics()->QuadsBegin();
		RenderTools()->SelectSprite(SPRITE_HELMET_BASEBALL, Direction.x > 0 ? SPRITE_FLAG_FLIP_X : 0);
	...
	else if (Player.m_CrateBuff == CRATE_FREEZE)
	{
		Graphics()->TextureSet(g_pData->m_aImages[IMAGE_FX_ICE].m_Id);
		Graphics()->QuadsBegin();
		RenderTools()->SelectSprite(SPRITE_FX_ICE_0 + Player.m_CrateBuffMisc);

		int AnimState = (Client()->GameTick() - Player.m_CrateBuffTick);
		Graphics()->SetColor(1, 1, 1, 0.5f);
		IGraphics::CQuadItem QuadItem(Position.x, Position.y - 16, (AnimState <= 10 ? AnimState / 10.0f : 1) * 96, (AnimState <= 10 ? AnimState / 10.0f : 1) * 96);
		Graphics()->QuadsDraw(&QuadItem, 1);
		Graphics()->QuadsEnd();
	}
	...
	{
		int Emote = m_pClient->m_aClients[ClientID].m_Emoticon;

		if (m_pClient->m_aClients[ClientID].m_Emoticon < 16)
			Graphics()->TextureSet(g_pData->m_aImages[IMAGE_EMOTICONS].m_Id);
		else
		{
		...
		RenderTools()->SelectSprite((m_pClient->m_aClients[ClientID].m_Emoticon < 16 ? SPRITE_OOP : SPRITE_E_REACTOR) + Emote);
